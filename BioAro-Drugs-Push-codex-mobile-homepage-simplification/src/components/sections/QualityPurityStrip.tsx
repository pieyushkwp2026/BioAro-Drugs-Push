...
import type { ProductQualityBadge } from "../../lib/shopify/types";
...
export default function QualityPurityStrip({
  ...
  if (qualityBadges?.length) {
    const enabledBadges = qualityBadges.filter((item) => item.enabled);
      ...
      <section className="mt-12 glass-card border border-ink/8 bg-[rgba(255,255,255,0.62)] p-6 md:p-8">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
          {enabledBadges.map((item) => (
            <div
              key={item.title}
              className="flex min-h-[184px] flex-col items-center justify-center rounded-2xl border border-ink/8 bg-white/55 px-4 py-5 text-center shadow-[0_12px_30px_-24px_rgba(27,26,23,0.18)]"
            >
              <div className="flex h-11 w-11 items-center justify-center rounded-full bg-forest-600/10 text-forest-600">
                <ShieldCheck size={18} strokeWidth={2.2} />
              </div>
              <p className="mt-4 text-[12px] font-semibold uppercase tracking-[0.14em] text-ink/80">{item.title}</p>
              {item.text ? <p className="mt-2 max-w-[13rem] text-xs leading-relaxed text-ink/50">{item.text}</p> : null}
            </div>
          ))}
        </div>
      </section>
