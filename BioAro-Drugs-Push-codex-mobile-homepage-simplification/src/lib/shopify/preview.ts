...
import { getMarketConfigByCountry, isProductAvailableInMarket, isProductComingSoonInMarket, isProductDraftInMarket, marketFromCountryCode } from "../../config/markets";
...
function isVisibleInMarket(handle: string, country: CountryCode) {
...
function isResolvableInMarket(handle: string, country: CountryCode) {
  const market = marketFromCountryCode(country);
  return isVisibleInMarket(handle, country) || isProductDraftInMarket(handle, market);
}
...
export function buildPreviewCatalog(country: CountryCode): CatalogProduct[] {
  return PREVIEW_PRODUCTS
    .filter((product) => isVisibleInMarket(product.handle, country))
    .map((product) => buildCatalogProduct(product, country));
}
...
export function buildPreviewProductByHandle(handle: string, country: CountryCode) {
  ...
  return product && isResolvableInMarket(product.handle, country) ? buildCatalogProduct(product, country) : undefined;
}
